...
var Parser = /** @class */ (function () {
    function Parser(options, document, fragmentContext, scriptHandler) {
        if (fragmentContext === void 0) { fragmentContext = null; }
        if (scriptHandler === void 0) { scriptHandler = null; }
        ...
        this.options = __assign(__assign({}, defaultParserOptions), options);
    ...
    // API
    Parser.parse = function (html, options) {
        var parser = new this(options);
        parser.tokenizer.write(html, true);
        return parser.document;
    };
    Parser.getFragmentParser = function (fragmentContext, options) {
        var opts = __assign(__assign({}, defaultParserOptions), options);
    ...
    Parser.prototype.getFragment = function () {
    ...
    //Errors
    Parser.prototype._err = function (token, code, beforeToken) {
        var _a;
        if (!this.onParseError)
            return;
    ...
    //Stack events
    Parser.prototype.onItemPush = function (node, tid, isTop) {
        var _a, _b;
        (_b = (_a = this.treeAdapter).onItemPush) === null || _b === void 0 ? void 0 : _b.call(_a, node);
        if (isTop && this.openElements.stackTop > 0)
            this._setContextModes(node, tid);
    };
    Parser.prototype.onItemPop = function (node, isTop) {
        var _a;
        var _b, _c;
        if (this.options.sourceCodeLocationInfo) {
            this._setEndLocation(node, this.currentToken);
        }
        (_c = (_b = this.treeAdapter).onItemPop) === null || _c === void 0 ? void 0 : _c.call(_b, node, this.openElements.current);
        if (isTop) {
            var current = void 0;
            ...
            if (this.openElements.stackTop === 0 && this.fragmentContext) {
                ...
                (_a = this.openElements, current = _a.current, currentTagId = _a.currentTagId);
            }
            this._setContextModes(current, currentTagId);
        }
    };
    Parser.prototype._setContextModes = function (current, tid) {
    ...
    Parser.prototype._setDocumentType = function (token) {
        ...
        if (token.location) {
            ...
            var docTypeNode = documentChildren.find(function (node) { return _this.treeAdapter.isDocumentTypeNode(node); });
            if (docTypeNode) {
                this.treeAdapter.setNodeSourceCodeLocation(docTypeNode, token.location);
            }
        }
    };
    Parser.prototype._attachElementToTree = function (element, location) {
        if (this.options.sourceCodeLocationInfo) {
            var loc = location && __assign(__assign({}, location), { startTag: location });
        ...
        if (this._shouldFosterParentOnInsertion()) {
            ...
            var parent_1 = this.openElements.currentTmplContentOrNode;
            this.treeAdapter.appendChild(parent_1, element);
        }
    };
    ...
    Parser.prototype._insertFakeElement = function (tagName, tagID) {
    ...
    Parser.prototype._insertCharacters = function (token) {
        ...
        var beforeElement;
        if (this._shouldFosterParentOnInsertion()) {
            (_a = this._findFosterParentingLocation(), parent = _a.parent, beforeElement = _a.beforeElement);
            if (beforeElement) {
        ...
        if (tnLoc) {
            var _b = token.location, endLine = _b.endLine, endCol = _b.endCol, endOffset = _b.endOffset;
    ...
    Parser.prototype._setEndLocation = function (element, closingToken) {
    ...
    //Token processing
    Parser.prototype.shouldProcessStartTagTokenInForeignContent = function (token) {
        var _a;
    ...
    //Integration points
    Parser.prototype._isIntegrationPoint = function (tid, element, foreignNS) {
    ...
    Parser.prototype._reconstructActiveFormattingElements = function () {
        ...
        if (listLength) {
            var endIndex = this.activeFormattingElements.entries.findIndex(function (entry) { return entry.type === formatting_element_list_js_1.EntryType.Marker || _this.openElements.contains(entry.element); });
    ...
    //Foster parenting
    Parser.prototype._isElementCausesFosterParenting = function (tn) {
        ...
        for (var i = this.openElements.stackTop; i >= 0; i--) {
            ...
            switch (this.openElements.tagIDs[i]) {
                case html_js_1.TAG_ID.TEMPLATE:
                    ...
                    break;
                case html_js_1.TAG_ID.TABLE: {
                    var parent_2 = this.treeAdapter.getParentNode(openElement);
                    if (parent_2) {
                        return { parent: parent_2, beforeElement: openElement };
                    }
    ...
    //Special elements
    Parser.prototype._isSpecialElement = function (element, id) {
        var ns = this.treeAdapter.getNamespaceURI(element);
        return html_js_1.SPECIAL_ELEMENTS[ns].has(id);
    };
    Parser.prototype.onCharacter = function (token) {
        this.skipNextNewLine = false;
    ...
    Parser.prototype.onNullCharacter = function (token) {
        this.skipNextNewLine = false;
        if (this.tokenizer.inForeignNode) {
    ...
    Parser.prototype.onComment = function (token) {
        this.skipNextNewLine = false;
        if (this.currentNotInHTML) {
    ...
    Parser.prototype.onDoctype = function (token) {
        this.skipNextNewLine = false;
        switch (this.insertionMode) {
            case InsertionMode.INITIAL:
    ...
    Parser.prototype.onStartTag = function (token) {
        this.skipNextNewLine = false;
     ...
     */
    Parser.prototype._processStartTag = function (token) {
    ...
    Parser.prototype._startTagOutsideForeignContent = function (token) {
        switch (this.insertionMode) {
            case InsertionMode.INITIAL:
                tokenInInitialMode(this, token);
                break;
    ...
    Parser.prototype.onEndTag = function (token) {
        this.skipNextNewLine = false;
        this.currentToken = token;
        if (this.currentNotInHTML) {
    ...
    Parser.prototype._endTagOutsideForeignContent = function (token) {
        switch (this.insertionMode) {
            case InsertionMode.INITIAL:
                tokenInInitialMode(this, token);
                break;
    ...
    Parser.prototype.onEof = function (token) {
        switch (this.insertionMode) {
            case InsertionMode.INITIAL:
                tokenInInitialMode(this, token);
                break;
    ...
    Parser.prototype.onWhitespaceCharacter = function (token) {
