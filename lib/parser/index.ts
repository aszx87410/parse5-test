...
import { OpenElementStack, type StackHandler } from "./open-element-stack";
import {
  FormattingElementList,
  EntryType,
  type ElementEntry,
} from "./formatting-element-list";
import {
  defaultTreeAdapter,
  type DefaultTreeAdapterMap,
} from "../tree-adapters/default";
import * as doctype from "../common/doctype";
import * as foreignContent from "../common/foreign-content";
import { ERR, type ParserErrorHandler } from "../common/error-codes";
import * as unicode from "../common/unicode";
import {
  TAG_ID as $,
  TAG_NAMES as TN,
  NS,
  ...
  implements TokenHandler, StackHandler<T>
{
  ...
  // API
  public static parse<T extends TreeAdapterTypeMap>(
    html: string,
    options?: ParserOptions<T>
  ): T["document"] {
  ...
  public getFragment(): T["documentFragment"] {
    const rootElement = this.treeAdapter.getFirstChild(
      this.document
    ) as T["parentNode"];
  ...
  _reconstructActiveFormattingElements(): void {
    ...
    if (listLength) {
      ...
      for (let i = unopenIdx; i >= 0; i--) {
        const entry = this.activeFormattingElements.entries[
          i
        ] as ElementEntry<T>;
        this._insertElement(
          entry.token,
...
function aaInnerLoop<T extends TreeAdapterTypeMap>(
  ...
  let nextElement = p.openElements.getCommonAncestor(
    furthestBlock
  ) as T["element"];
...
function callAdoptionAgency<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
//------------------------------------------------------------------
function appendComment<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: CommentToken
): void {
...
function appendCommentToRootHtmlElement<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: CommentToken
): void {
...
function appendCommentToDocument<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: CommentToken
): void {
...
function stopParsing<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: EOFToken
): void {
  p.stopped = true;
...
//------------------------------------------------------------------
function doctypeInInitialMode<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: DoctypeToken
): void {
...
function tokenInInitialMode<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: Token
): void {
...
//------------------------------------------------------------------
function startTagBeforeHtml<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (token.tagID === $.HTML) {
...
function endTagBeforeHtml<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function tokenBeforeHtml<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: Token
): void {
...
//------------------------------------------------------------------
function startTagBeforeHead<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function endTagBeforeHead<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  const tn = token.tagID;
...
function tokenBeforeHead<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: Token
): void {
...
//------------------------------------------------------------------
function startTagInHead<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function endTagInHead<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function templateEndTagInHead<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function tokenInHead<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: Token
): void {
  p.openElements.pop();
...
//------------------------------------------------------------------
function startTagInHeadNoScript<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function endTagInHeadNoScript<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function tokenInHeadNoScript<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: Token
): void {
  const errCode =
...
//------------------------------------------------------------------
function startTagAfterHead<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function endTagAfterHead<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function tokenAfterHead<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: Token
): void {
...
//------------------------------------------------------------------
function modeInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: Token
): void {
  switch (token.type) {
...
function characterInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: CharacterToken
): void {
  p._reconstructActiveFormattingElements();
...
function htmlStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (p.openElements.tmplCount === 0) {
...
function bodyStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function framesetStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  const bodyElement = p.openElements.tryPeekProperlyNestedBodyElement();
...
function addressStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function numberedHeaderStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (p.openElements.hasInButtonScope($.P)) {
...
function preStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (p.openElements.hasInButtonScope($.P)) {
...
function formStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function listItemStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p.framesetOk = false;
...
function plaintextStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (p.openElements.hasInButtonScope($.P)) {
...
function buttonStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function aStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function bStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p._reconstructActiveFormattingElements();
...
function nobrStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p._reconstructActiveFormattingElements();
...
function appletStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p._reconstructActiveFormattingElements();
...
function tableStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (
...
function areaStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p._reconstructActiveFormattingElements();
...
function inputStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p._reconstructActiveFormattingElements();
...
function paramStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p._appendElement(token, NS.HTML);
...
function hrStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (p.openElements.hasInButtonScope($.P)) {
...
function imageStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function textareaStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p._insertElement(token, NS.HTML);
...
function xmpStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (p.openElements.hasInButtonScope($.P)) {
...
function iframeStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p.framesetOk = false;
...
//<noembed> as rawtext.
function noembedStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p._switchToTextParsing(token, TokenizerMode.RAWTEXT);
...
function selectStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p._reconstructActiveFormattingElements();
...
function optgroupStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function rbStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function rtStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (p.openElements.hasInScope($.RUBY)) {
...
function mathStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p._reconstructActiveFormattingElements();
...
function svgStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p._reconstructActiveFormattingElements();
...
function genericStartTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p._reconstructActiveFormattingElements();
...
function startTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function bodyEndTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function htmlEndTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (p.openElements.hasInScope($.BODY)) {
...
function addressEndTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  const tn = token.tagID;
...
function ddEndTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  const tn = token.tagID;
...
function numberedHeaderEndTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>
): void {
...
function appletEndTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  const tn = token.tagID;
...
function genericEndTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function endTagInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function eofInBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: EOFToken
): void {
...
//------------------------------------------------------------------
function endTagInText<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function eofInText<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: EOFToken
): void {
...
//------------------------------------------------------------------
function characterInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: CharacterToken
): void {
...
function captionStartTagInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function colgroupStartTagInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p.openElements.clearBackToTableContext();
...
function colStartTagInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p.openElements.clearBackToTableContext();
...
function tbodyStartTagInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p.openElements.clearBackToTableContext();
...
function tdStartTagInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  p.openElements.clearBackToTableContext();
...
function tableStartTagInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function inputStartTagInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (isHiddenInput(token)) {
...
function formStartTagInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function startTagInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function endTagInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function tokenInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: Token
): void {
...
//------------------------------------------------------------------
function whitespaceCharacterInTableText<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: CharacterToken
): void {
...
function characterInTableText<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: CharacterToken
): void {
  p.pendingCharacterTokens.push(token);
...
function tokenInTableText<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: Token
): void {
  let i = 0;
...
function startTagInCaption<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  const tn = token.tagID;
...
function endTagInCaption<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  const tn = token.tagID;
...
//------------------------------------------------------------------
function startTagInColumnGroup<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function endTagInColumnGroup<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function tokenInColumnGroup<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: Token
): void {
  if (p.openElements.currentTagId === $.COLGROUP) {
...
//------------------------------------------------------------------
function startTagInTableBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function endTagInTableBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  const tn = token.tagID;
...
//------------------------------------------------------------------
function startTagInRow<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function endTagInRow<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
//------------------------------------------------------------------
function startTagInCell<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  const tn = token.tagID;
...
function endTagInCell<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  const tn = token.tagID;
...
//------------------------------------------------------------------
function startTagInSelect<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function endTagInSelect<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
//------------------------------------------------------------------
function startTagInSelectInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  const tn = token.tagID;
...
function endTagInSelectInTable<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  const tn = token.tagID;
...
//------------------------------------------------------------------
function startTagInTemplate<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function endTagInTemplate<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function eofInTemplate<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: EOFToken
): void {
  if (p.openElements.tmplCount > 0) {
...
//------------------------------------------------------------------
function startTagAfterBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (token.tagID === $.HTML) {
...
function endTagAfterBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (token.tagID === $.HTML) {
...
function tokenAfterBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: Token
): void {
  p.insertionMode = InsertionMode.IN_BODY;
...
//------------------------------------------------------------------
function startTagInFrameset<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function endTagInFrameset<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (
...
//------------------------------------------------------------------
function startTagAfterFrameset<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
function endTagAfterFrameset<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (token.tagID === $.HTML) {
...
//------------------------------------------------------------------
function startTagAfterAfterBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  if (token.tagID === $.HTML) {
...
function tokenAfterAfterBody<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: Token
): void {
  p.insertionMode = InsertionMode.IN_BODY;
...
//------------------------------------------------------------------
function startTagAfterAfterFrameset<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
  switch (token.tagID) {
...
//------------------------------------------------------------------
function nullCharacterInForeignContent<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: CharacterToken
): void {
...
function popUntilHtmlOrIntegrationPoint<T extends TreeAdapterTypeMap>(
  p: Parser<T>
): void {
  while (
...
function startTagInForeignContent<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
...
function endTagInForeignContent<T extends TreeAdapterTypeMap>(
  p: Parser<T>,
  token: TagToken
): void {
